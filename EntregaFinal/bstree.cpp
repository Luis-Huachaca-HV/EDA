...
class BST_int{
    ...
    public:
        ...
        ***********************/
        ...
        void get_levelnodes(node *root, vector<node *> &rootlist, int level, int k){
            if (root == NULL){
                return;
            }
            get_levelnodes(root->left,rootlist, level+1, k);
            get_levelnodes(root->right,rootlist, level+1, k);
            if (k == level){

...
class RTree {
 public:
  ...
  struct Node {
    ...
    bool is_leaf();
    void delete_pos(const std::shared_ptr<Node> &current,size_t indexval ){
            size_t val_c = indexval-1;
            if(val_c != -1){
                //shift all the element from index+1 by one position to the left
                for(size_t i = indexval; i < size - 1; i++)
  ...
  void delet(const std::shared_ptr<Node> &current ,const Rectangle<N>& box, const ElemType& value);
  ...
  void printarbol(const std::shared_ptr<Node> &current_node, int index);
...
void RTree<N, ElemType, M, m>::Node::pickSeeds(
  ...
  for (size_t i = 0; i < M; ++i) {
    for (size_t j = i + 1; j < M + 1; ++j) {
      ...
      /*if (enlargement_box.get_area() < entries.at(i).box.get_area() - entries.at(j).box.get_area() )
...
void RTree<N, ElemType, M, m>::Node::pickNext(
              ...
              Rectangle<N>& mbb1, Rectangle<N>& mbg2) {
  float max_diff = -1.f;
  ...
  for (size_t i = 0; i < entries.size(); ++i) {
    ...
    area_increase_g1 = enlarged_g1.get_area() - mbb1.get_area();

    enlarged_g2 = mbg2;
    enlarged_g2.adjust(entries.at(i).box);

    area_increase_g2 = enlarged_g2.get_area() - mbg2.get_area();
...
RTree<N, ElemType, M, m>::Node::insert(const SpatialObject &new_entry) {
  ...
  while (!entries.empty()) {
    if (this->size + entries.size() == m) {
      ...
      pickNext(entries, &idx, mbb1, mbg2);

      mbb_readj_g1 = mbb1;
      mbb_readj_g1.adjust(entries.at(idx).box);
      area_increase_g1 = mbb_readj_g1.get_area() - mbb1.get_area();

      mbb_readj_g2 = mbg2;
      mbb_readj_g2.adjust(entries.at(idx).box);
      area_increase_g2 = mbb_readj_g2.get_area() - mbg2.get_area();

      if (area_increase_g1 < area_increase_g2 ||
          (area_increase_g1 == area_increase_g2 &&
           mbb1.get_area() < mbg2.get_area()) ||
          (area_increase_g1 == area_increase_g2 &&
           mbb1.get_area() == mbg2.get_area() &&
           size <= new_node->size)) {
        mbb1.adjust(entries.at(idx).box);
        this->insert(entries.at(idx));
      } else {
        mbg2.adjust(entries.at(idx).box);
...
template <size_t N, typename ElemType, size_t M, size_t m>
void RTree<N, ElemType, M, m>::printarbol(const std::shared_ptr<Node> &current_node, int index) {

  std::cout << "nivel: " << index << "\n" << "\n";
  for (SpatialObject current_entry : *current_node) {

      std::cout << current_entry.identifier << "\t";
      std::shared_ptr<Node> next_entry = current_entry.child_pointer;

      if(current_entry.child_pointer)
      {
        std::cout << "\n";
        printarbol(current_entry.child_pointer, index + 1);
      }
...
//Intento de delete, no funcional de momento.
template <size_t N, typename ElemType, size_t M, size_t m>
void RTree<N, ElemType, M, m>::delet(const std::shared_ptr<Node> &current ,const Rectangle<N>& box, const ElemType& value) {
  
    size_t indexval=0;
    for(SpatialObject current_entry : *current){
        if (current_entry.identifier == value){
            //current_entry[inde]
            (*current).delete_pos(current,indexval );
            //std::cout << current_entry.identifier  << std::endl;
            //current_entry->remove(indexval);
        }
        if (current_entry.child_pointer)
            delet(current_entry.child_pointer, box,value);
        indexval = indexval+1;
    }
}
